...
- show: **v1.12(2026-08-22)부터 매일 true**(구 v1.4~v1.11: 월~금만 true, 토·일 false → UI 미노출 — 사장님 8/22 09:18 "로또 번호 안나오네" 지적으로 요일 제한 폐지). UI는 오늘의운세 카드 하단 공 모양 번호 6개.
...
- **재방문**(프로필 있음): **홈 대시보드** 표시 —
  ...
  ③ 로또 번호 카드(v1.4 재사용, v1.12부터 매일 노출)
...
- 로또 섹션이 미노출인 날(v1.4 `show` 규칙)에는 기록도 하지 않는다 — 화면에 나타난 번호만 쌓인다는 원칙 유지. (v1.12부터 `show` 는 항상 true이므로 매일 기록된다.)
...
## v1.12 추가: 로또 번호 주 7일 매일 노출 (2026-08-22 사장님 지시 "로또 번호 안나오네" — 토요일 미노출 문제)
- **문제**: v1.4 `ContentDB.lotto()` 의 `show` 가 `lottoIsWeekday(dateStr)` (월~금만 true) 로 판정되어 토·일에는 로또 카드 자체가 렌더되지 않았다(결과 화면·홈 대시보드 공통). v1.11 누적 기록도 `show !== true` 면 기록을 건너뛰므로 주말은 기록도 비어 있었다.
- **수정**: `content-db.js` `lotto()` 반환값의 `show` 를 항상 `true` 로 고정. 요일 판정 헬퍼 `lottoIsWeekday()`(및 `_internal.lottoIsWeekday` 노출)는 더 이상 쓰이지 않아 제거. 번호 생성(시드=chart+날짜, 오행 가중치) 로직은 무변경 — 토·일도 해당 날짜 시드로 정상 계산된다.
- **UI**: `index.html` 의 `renderLotto`/`renderDashLotto`/`recordLottoHistory` 는 모두 기존 `info.show !== true` 가드를 그대로 유지(향후 재도입 대비 방어 코드) — `show` 값 자체가 항상 true 가 되므로 결과 화면·홈 대시보드·누적 기록 모두 매일 노출·기록된다.
- **회귀**: `_verify_lotto.js`(요일 무관 show=true 전수 확인), `_e2e_lotto.js`(토요일도 카드 노출로 기대값 변경), `_e2e_lotto_history.js`(토요일 케이스 추가 — 노출+기록 동시 확인). 기존 결정성·범위·가중치 통계 검증은 무변경.

## v1.13 추가: 지난 번호 목록 최소 5줄 소급 채움 (2026-08-22 09:25 사장님 지시 "누적 5개는 보여야지")
- **문제**: v1.11 누적 기록은 프로필이 실제로 방문한 날짜만 쌓이므로, 신규 프로필이거나 며칠치만 방문한 경우 "지난 번호" 목록이 5줄에 한참 못 미치거나 비어 있었다.
- **수정**: `index.html` `renderLottoHistory()` — 저장된(오늘 제외) 기록이 `MIN_HISTORY_ROWS`(5) 미만이면 `backfillLottoHistoryRows()`가 어제부터 역순으로 `ContentDB.lotto(chart, 날짜)`를 재계산해 부족분을 채운다. 소급값은 "그 날짜에 실제로 접속했다면 나왔을 번호"와 동일한 결정적 재계산이라 허구가 아니다(v1.12로 요일 제한이 없어져 주말 날짜도 소급 대상에 포함된다).
  - 이미 저장된 날짜와 소급 계산 날짜가 겹치면 저장값을 그대로 신뢰(우선)하고, 값이 다르면(정상 흐름에서는 발생하지 않아야 함) `console.warn`만 남긴다.
  - 소급으로 채운 날짜도 `LottoHistoryStore.record()`로 즉시 저장해, 다음부터는 재계산 없이 저장된 기록으로 뜬다(하루 1줄 정책과 동일하게 중복 없음).
  - 날짜 연산은 로컬 날짜 getter로 포맷한다(`TODAY_STR`과 동일 방식) — `toISOString()`은 UTC 변환 때문에 KST 등 양의 UTC 오프셋 환경에서 하루가 밀리는 버그가 있어 사용하지 않는다(구현 중 실제로 재현·수정).
- **오늘 번호**는 기존과 동일하게 위 "오늘의 로또 번호" 카드에만 표시되고 목록에서는 계속 제외된다.
- **회귀**: `_e2e_lotto_history.js` 갱신 — A일차(신규 프로필, 기록 0건) 첫 방문에서 목록이 즉시 5줄(소급, 주말 포함)로 채워짐을 확인하고, 이미 5줄 이상 쌓인 뒤에는 추가 소급이 없음(B일차)·같은 날 재조회 시 중복 소급이 없음(2단계)을 확인. 토요일 케이스(별도 프로필)도 동일하게 6건(오늘 1+소급 5)으로 갱신.
